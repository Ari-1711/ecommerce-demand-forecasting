...
export default function KpiGrid({ kpis, metrics, loading }) {
  if (loading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="saas-card p-4 sm:p-5 rounded-xl animate-pulse h-32 flex flex-col justify-between">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4">
      ...
      <div className="saas-card p-4 sm:p-5 rounded-xl flex flex-col justify-between">
        ...
        <div className="flex items-center text-[11px] text-slate-400">
          <Activity className="w-3 h-3 text-indigo-400 mr-1.5 flex-shrink-0" /> Total permintaan seluruh SKU
        </div>
      </div>
      ...
      <div className="saas-card p-4 sm:p-5 rounded-xl flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between text-[11px] pt-1 gap-1">
          <span className="text-rose-400 font-medium bg-rose-500/10 px-1.5 py-0.5 rounded border border-rose-500/20 truncate">
            A: {kpis?.sku_kelas_a || 0}
          </span>
          <span className="text-amber-400 font-medium bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20 truncate">
            B: {kpis?.sku_kelas_b || 0}
          </span>
          <span className="text-cyan-400 font-medium bg-cyan-500/10 px-1.5 py-0.5 rounded border border-cyan-500/20 truncate">
      ...
      <div className="saas-card p-4 sm:p-5 rounded-xl flex flex-col justify-between border-indigo-500/20">
        ...
        <div className="grid grid-cols-3 gap-1.5 my-1">
          <div className="bg-slate-900/80 p-1.5 rounded text-center border border-slate-800">
            ...
            <div className="text-xs sm:text-sm font-bold font-mono text-emerald-400">{metrics?.mae ?? 86.41}</div>
          </div>
          <div className="bg-slate-900/80 p-1.5 rounded text-center border border-slate-800">
            <div className="text-[9px] text-slate-400 uppercase font-medium">R²</div>
            <div className="text-xs sm:text-sm font-bold font-mono text-indigo-400">{metrics?.r2_percentage ?? '45.53%'}</div>
          </div>
          <div className="bg-slate-900/80 p-1.5 rounded text-center border border-slate-800">
            <div className="text-[9px] text-slate-400 uppercase font-medium">RMSE</div>
            <div className="text-xs sm:text-sm font-bold font-mono text-amber-400">{metrics?.rmse ?? 206.56}</div>
          </div>
        </div>
